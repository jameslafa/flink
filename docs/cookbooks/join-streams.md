...
Like in relational databases, we avoid duplicating data in streams to improve performances. However, we need sometime to add external context to data flowing in a stream. In a relational data we would JOIN two tables. In Flink we join two streams based on a key to generate a stream of enriched data.

A common pattern is change data capture (CDC). We capture data modifications pushed in a stream and save them in the state. When data flow in the second stream, we lookup the state to get the related context and enrich the flowing data. This technique make it possible to execute a lookup at very low latency in the state instead of a slow lookup in an external source like a database.


## Use case

To illustrate this technique, we will take as example a company running multiple advertisement campaigns which would like to display analytics in a dashboard.
...
We need to connect streams before joining them. Two streams are connected via a common key. In our example it is AdCampaign.id and AdClick.campaignId.
...
In our case, we want to enrich data flowing in AdClickStream with data flowing in AdCampaignStream. Therefore, we must capture AdCampaign changes in the state which will later be used to lookup AdCampaigns details.
...
Let's create a new function extending RichCoFlatMapFunction. We must specific three type parameters:
 ...
 3. the data type which will be collected by the collector and pushed down stream: EnrichedAdClick
...
`flapMap1` is executed when new elements are flowing in the first stream: AdClickStream. For each flowing AdClick we read the corresponding AdCampaign information from the MapState and create a new EnrichedAdClick that we push down the stream via the collector.
...
That's it! The flatMap operator returns now a stream of EnrichedAdClick. You can apply other operators of the DataStreamApi.
